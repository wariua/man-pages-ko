...
`_exit()`는 호출 프로세스를 "즉시" 종료시킨다. 그 프로세스에게 속한 열린 파일 디스크립터가 있으면 모두 닫힌다. 그 프로세스의 자식들이 있으면 `init(1)`에게 (또는 <tt>[[prctl(2)]]</tt> `PR_SET_CHILD_SUBREAPER` 동작을 통해 지정한 가장 가까운 "서브리퍼" 프로세스에게) 인계된다. 호출 프로세스의 부모에게 `SIGCHLD` 시그널이 전송된다.

부모 프로세스에게 프로세스 종료 상태로 `status & 0xFF` 값이 반환되며, 부모에서 <tt>[[wait(2)]]</tt> 계열 함수들 중 하나를 이용해 그 값을 수집할 수 있다.
...
glibc 버전 2.3 전까지는 `_exit()` 래퍼 함수가 같은 이름의 커널 시스템 호출을 불렀다. glibc 2.3부터는 프로세스의 모든 스레드를 종료시키도록 래퍼 함수에서 <tt>[[exit_group(2)]]</tt>을 부른다. (`_exit()` 시스템 호출은 호출 스레드만 종료시킨다.)

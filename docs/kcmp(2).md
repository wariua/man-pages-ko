...
`KCMP_FILES`
:   프로세스들이 동일한 열린 파일 디스크립터 집합을 공유하는지 확인한다. `idx1` 및 `idx2` 인자는 무시한다. <tt>[[clone(2)]]</tt>의 `CLONE_FILES` 플래그 논의 참고.

`KCMP_FS`
:   프로세스들이 같은 파일 시스템 정보(즉 파일 모드 생성 마스크, 작업 디렉터리, 파일 시스템 루트)를 공유하는지 확인한다. `idx1` 및 `idx2` 인자는 무시한다. <tt>[[clone(2)]]</tt>의 `CLONE_FS` 플래그 논의 참고.

`KCMP_IO`
:   프로세스들이 I/O 문맥을 공유하는지 확인한다. `idx1` 및 `idx2` 인자는 무시한다. <tt>[[clone(2)]]</tt>의 `CLONE_IO` 플래그 논의 참고.

`KCMP_SIGHAND`
:   프로세스들이 동일한 시그널 처리 방식 테이블을 공유하는지 확인한다. `idx1` 및 `idx2` 인자는 무시한다. <tt>[[clone(2)]]</tt>의 `CLONE_SIGHAND` 플래그 논의 참고.

`KCMP_SYSVSEM`
:   프로세스들이 동일한 시스템 V 세마포어 작업 취소 목록을 공유하는지 확인한다. `idx1` 및 `idx2` 인자는 무시한다. <tt>[[clone(2)]]</tt>의 `CLONE_SYSVSEM` 플래그 논의 참고.

`KCMP_VM`
:   프로세스들이 동일한 주소 공간을 공유하는지 확인한다. `idx1` 및 `idx2` 인자는 무시한다. <tt>[[clone(2)]]</tt>의 `CLONE_VM` 플래그 논의 참고.
...
0
:   `v1`이 `v2`와 같다. 다시 말해 두 프로세스가 자원을 공유한다.

...
바이너리 모드 구현에는 API 버그가 있었는데, 바이너리 모드를 지정하려면 'b'가 `mode`에서 <em>두 번째</em> 문자여야 한다. 그래서 예를 들어 "wb+"라고 하면 원하는 효과가 나지만 "w+b"라고 하면 나지 않는다. <tt>[[fopen(3)]]</tt>의 `mode` 처리 방식과 일치하지 않는 방식이다.

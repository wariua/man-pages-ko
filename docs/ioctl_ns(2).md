...
네임스페이스 관계(<tt>[[user_namespaces(7)]]</tt> 및 <tt>[[pid_namespaces(7)]]</tt> 참고)를 알아낼 수 있도록 다음 <tt>[[ioctl(2)]]</tt> 동작을 제공한다. 호출 형태는 이렇다.
...
어느 쪽 <tt>[[ioctl(2)]]</tt> 동작이든 다음 오류로 실패할 수 있다.
...
아래 있는 예시 프로그램에서는 위에 설명한 <tt>[[ioctl(2)]]</tt> 동작들을 사용해 간단한 네임스페이스 관계 탐색을 수행한다. 다음 셸 세션들은 이 프로그램의 다양한 사용 방식을 보여 준다.
...
<tt>[[fstat(2)]]</tt>, <tt>[[ioctl(2)]]</tt>, <tt>[[proc(5)]]</tt>, <tt>[[namespaces(7)]]</tt>

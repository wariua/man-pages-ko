...
* 다양한 버그들이 의미하는 바는 권한 검사에 있어서 두 `tv_nsec` 필드가 모두 `UTIME_NOW`로 설정된 경우를 항상 `times`를 NULL로 지정한 것과 같게 다루지는 않는다는 것, 그리고 한 `tv_nsec` 값이 `UTIME_NOW`이고 다른 값이 `UTIME_OMIT`인 경우를 `times`를 임의 시간 값들을 담은 구조체 배열의 포인터로 지정한 것과 같게 다루지 않는다는 것이다. 버그들 때문에 일부 경우에서 a) 갱신을 수행할 권한이 없어야 하는 프로세스가 파일 타임스탬프를 갱신할 수 있고, b) 갱신을 수행할 권한이 있어야 하는 프로세스가 파일 타임스탬프를 갱신할 수 없고, c) 오류 경우에 틀린 `errno` 값을 반환한다.

...
| `%E` | 수식자: 다른 형식 사용. 아래 참고. (SU) |
| `%F` | `%Y-%m-%d`와 같음. (ISO 8601 날짜 형식) (C99) |
| `%G` | ISO 8601 주 기준의 세기 포함한 십진수로 된 연도. (NOTES 참고.) 4자리 연도가 ISO 주 번호(`%V` 참고)에 부합한다. 그래서 `%Y`와 형식 및 값이 같되 ISO 주 번호가 지난해나 다음해에 속하면 그 연도를 쓴다. (TZ) (`tm_year`, `tm_yday`, `tm_wday`로 계산) |

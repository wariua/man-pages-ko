...
:   이 매개변수 설정은 다양한 프로그래밍 오류들(가령 같은 포인터 두 번 해제하기)을 탐지했을 때 glibc가 어떻게 대응하는지를 제어한다. 이 매개변수에 할당한 값의 하위 3개 (0번, 1번, 2번) 비트가 다음과 같이 glibc 동작을 결정한다.
    ...
    0
    :   오류 조건을 무시한다. (규정되지 않은 결과로) 실행을 계속한다.

    1
    :   자세한 오류 메시지를 찍고 실행을 계속한다.

    2
    :   프로그램 실행을 중단한다.

    3
    :   자세한 오류 메시지와 스택 트레이스, 메모리 매핑을 찍고 프로그램 실행을 중단한다.

    5
    :   간단한 오류 메시지를 찍고 실행을 계속한다.

    7
    :   간단한 오류 메시지와 스택 트레이스, 메모리 매핑을 찍고 프로그램 실행을 중단한다.

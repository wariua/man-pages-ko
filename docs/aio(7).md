...
<tt>[[aio_read(3)]]</tt>
:   읽기 요청을 큐에 넣는다. <tt>[[read(2)]]</tt>의 비동기 형태다.

<tt>[[aio_write(3)]]</tt>
:   쓰기 요청을 큐에 넣는다. <tt>[[write(2)]]</tt>의 비동기 형태다.
...
제어 블록 버퍼를 사용하기 전에 0으로 채우는 게 좋다. (<tt>[[memset(3)]]</tt> 참고.) I/O 동작이 진행 중인 동안 제어 블록 버퍼와 `aio_buf`가 가리키는 버퍼가 변경되어선 안 된다. I/O 동작이 완료될 때까지 그 버퍼들이 유효한 상태로 유지돼야 한다.

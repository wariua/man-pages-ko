...
함수 `drand48()`, `erand48()`, `lrand48()`, `nrand48()`, `mrand48()`, `jrand48()` 중 뭔가로 반환하는 값을 계산하기 위해 먼저 열의 다음 48비트 `Xi`를 생성한다. 그리고 반환할 데이터 항목의 타입에 따라 적절한 수의 비트를 `Xi`의 상위 비트들에서 복사해서 반환 값으로 변환한다.

`drand48()`, `lrand48()`, `mrand48()` 함수에서는 마지막으로 생성한 48비트 `Xi`를 내부 버퍼에 저장해 둔다. `erand48()`, `nrand48()`, `jrand48()` 함수에서는 연속한 `Xi` 값을 저장할 공간을 호출 프로그램이 `xsubi` 배열 인자로 제공해야 한다. 처음 함수를 호출하기 전에 그 배열에 `Xi`의 초기값을 넣어서 그 함수들을 초기화한다.

초기화 함수 `srand48()`은 `Xi`의 상위 32비트에 인자 `seedval`을 설정한다. 하위 16비트는 임의 값 0x330E로 설정한다.
...
위 함수들은 난수 생성기를 위한 전역 상태 정보를 기록하며, 따라서 스레드에 안전하지 않다.

...
부팅 때 OS 이미지 상의 시스템 관리자는 시스템 동작에 필요한 필수 파일 시스템과 서비스, 드라이버의 초기화를 맡는다. `systemd(1)` 시스템에서 이 과정은 다양한 분리된 단계들로 쪼개져서 타깃 유닛들로 노출되어 있다. (타깃 유닛에 대한 자세한 정보는 `systemd.target(5)`을 보라.) 부팅 과정이 고도로 병렬화되어 있어서 특정 타깃 유닛들에 도달하는 순서가 확정적이지 않다. 하지만 그래도 제한된 양의 순서 구조를 충실히 따른다.

systemd가 시스템을 구동할 때 default.target이 의존하는 모든 유닛들을 (그리고 그 의존 유닛들이 의존하는 것들까지 재귀적으로) 활성화한다. 일반적으로 default.target은 graphical.target이나 multi-user.target의 에일리어스일 뿐이며, 어느 쪽인지는 시스템이 그래픽 UI로 구성되어 있는지 텍스트 콘솔로만 구성되어 있는지에 따라 결정된다. 당겨 오는 유닛들 간에 최소한의 순서 관계만을 강제하기 위해 잘 알려진 타깃 유닛들이 여럿 있다. `systemd.special(7)`에 나열되어 있다.
...
systemd에서의 시스템 정지 역시 다양한 타깃 단위들로 이뤄져 있으며 어떤 최소한의 순서 구조가 적용된다.

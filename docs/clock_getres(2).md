...
`clock_getres()` 함수는 지정한 클럭 `clockid`의 해상도(정밀도)를 알아내고, `res`가 NULL이 아니면 `res`가 가리키는 `struct timespec`에 그 해상도를 저장한다. 클럭의 해상도는 구현체에 따라 달라지며 특정 프로세스가 설정할 수 있는 것이 아니다. `clock_settime()`의 인자 `tp`가 가리키는 시간 값이 `res`의 배수가 아니면 배수가 되게 잘라낸다.

함수 `clock_gettime()` 및 `clock_settime()`은 지정한 클럭 `clockid`의 시간을 가져오거나 설정한다.
...
`clockid` 인자는 동작을 수행할 구체적 클럭의 식별자이다. 클럭은 시스템 전역이어서 모든 프로세스에게 보일 수도 있고 프로세스별로 있어서 한 프로세스 내에서만 시간을 측정할 수도 있다.
...
:   `CLOCK_REALTIME`의 더 빠르지만 덜 정확한 버전. 이 클럭은 설정 가능하지 않다. 아주 빠르되 정밀하지는 않은 타임스탬프가 필요할 때 쓰면 된다. 아키텍처별 지원이 필요하며, <tt>[[vdso(7)]]</tt> 내에서 이 플래그에 대한 아키텍처 지원도 필요할 것이다.

`CLOCK_TAI` (리눅스 3.10부터. 리눅스 전용.)
:   벽시계 시간에서 파생되었고 윤초를 무시하는 설정 불가능한 시스템 전역 클럭. 이 클럭은 `CLOCK_REALTIME`처럼 NTP의 윤초 삽입으로 인한 불연속과 역방향 도약을 겪지 않는다.

    TAI는 국제원자시(International Atomic Time)를 나타낸다.

`CLOCK_MONOTONIC`
:   "어떤 규정돼 있지 않은 시점"(POSIX의 서술)부터 단조 증가하는 시간을 나타내는 설정 불가능한 시스템 전역 클럭. 리눅스에서는 시스템이 부팅하고 동작한 초 수에 해당한다.
...
위에서 설명한 하드코딩된 시스템 V 방식 클럭 ID에 더해서 리눅스에선 특정 문자 장치들에 대한 POSIX 클럭 동작도 지원한다. 그 장치를 "동적" 클럭이라고 하며 리눅스 2.6.39부터 지원한다.
...
`EINVAL` (리눅스 4.3부터)
:   `clockid`를 `CLOCK_REALTIME`으로 한 `clock_settime()` 호출에서 시간을 `CLOCK_MONOTINIC` 클럭 현재 값보다 작은 값으로 설정하려 했다.
...
> `clock_settime()`을 통해 `CLOCK_REALTIME` 클럭의 값을 설정하는 것이 `nanosleep()`을 포함해 이 클럭 기반의 상대적 시간 서비스를 기다리며 블록되어 있는 스레드들에 어떤 영향도 끼치지 않아야 하며, 이 클럭 기반의 상대적 타이머의 만료에 대해서도 마찬가지이다. 따라서 이런 시간 서비스들은 그 클럭의 신규 내지 이전 값과 상관없이 요청했던 상대적 시간이 경과했을 때에 만료되어야 한다.
...
아래 프로그램은 다양한 클럭으로 `clock_gettime()` 및 `clock_getres()` 사용 방식을 보여 준다. 다음은 프로그램 실행 시 볼 수 있을 출력 예이다.

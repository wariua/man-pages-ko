...
`CLOCK_MONOTONIC`
:   "어떤 규정돼 있지 않은 시점"(POSIX의 서술)부터 단조 증가하는 시간을 나타내는 설정 불가능한 시스템 전역 클럭. 리눅스에서는 시스템이 부팅 하고 동작한 초 수에 해당한다.
...
위에서 설명한 하드코딩 된 시스템 V 방식 클럭 ID에 더해서 리눅스에선 특정 문자 장치들에 대한 POSIX 클럭 동작도 지원한다. 그 장치를 "동적" 클럭이라고 하며 리눅스 2.6.39부터 지원한다.
...
`EINVAL`
:   지정한 `clockid`가 유효하지 않다. 시스템 V 방식의 하드코딩 된 양수 값이 범위를 벗어났거나, 동적인 클럭 ID가 유효한 클럭 객체 인스턴스를 가리키고 있지 않다.
